...
//! `wallet_rpc_core::Engine` (the same engine the USB server wraps) through the
//! full desktop-wallet flow: authorize a session, fetch an xpub, request an
//! ownership proof, sign a coinjoin PSBT, and confirm a foreign-coordinator
//! proof is rejected. This exercises the one piece unit tests can't: the
//! security-server → BIP-39 seed derivation that produces the signing key on
//! real hardware.
...
use ngwallet::bdk_wallet::{
    bitcoin::{
        absolute::LockTime, bip32::{ChildNumber, DerivationPath, Xpriv}, hashes::Hash, psbt::Psbt,
        secp256k1::{All, Secp256k1}, transaction::Version, Amount, CompressedPublicKey, Network,
...
/// server crate's `KeyOsBackend`) and auto-approves policies (no GUI in tests).
...
impl Backend for SecurityBackend {
    fn firmware_version(&self) -> String { "integration-test".into() }
    ...
    fn approve_policy(&mut self, _policy: &Policy) -> bool { true }
}
...
fn main() {
    ...
    assert_eq!(caps & 0b11, 0b11, "ownership + coinjoin capabilities");

    // Authorize a coinjoin session.
    let session_id =
        u32::from_le_bytes(expect_ok(&mut engine, CMD_AUTHORIZE_COINJOIN, &policy_bytes()).try_into().unwrap());
    log::info!("Session {session_id} authorized");
    ...
    // Ownership proof, checked against the functional core.
    ...
    let proof = expect_ok(
        ...
        &ownership_request(session_id, &path, &commit),
    );
    let expected =
        wallet_rpc_core::slip19::ownership_proof(&secp, &seed, Network::Bitcoin, &path, &commit, true)
            .unwrap();
    assert_eq!(proof, expected, "ownership proof matches functional core");

    // Sign a conforming coinjoin round.
    ...
    assert_eq!(signed_psbt.inputs[0].final_script_witness.is_some(), true, "our input signed");
    assert_eq!(signed_psbt.inputs[1].final_script_witness.is_none(), true, "foreign input untouched");

    // A proof committing to a foreign coordinator must be rejected.
    let evil = commitment(b"EvilCoordinator");
    let resp = engine.process_frame(&frame(CMD_GET_OWNERSHIP_PROOF, &ownership_request(session_id, &path, &evil)));
...
fn expect_ok(engine: &mut Engine<SecurityBackend>, cmd: u8, payload: &[u8]) -> Vec<u8> {
    ...
    assert_eq!(resp.len(), 5 + len, "response length");
    resp[5..].to_vec()
}
...
fn ownership_request(session_id: u32, path: &[u32], commitment: &[u8]) -> Vec<u8> {
    let mut payload = session_id.to_le_bytes().to_vec();
...
fn fixture_psbt(secp: &Secp256k1<All>, seed: &[u8]) -> Psbt {
    ...
    let derive = |path: &[u32]| {
        ...
        (pubkey, ScriptBuf::new_p2wpkh(&pubkey.wpubkey_hash()), derivation)
    };
    let (in_pk, in_spk, in_path) = derive(&[84 | H, H, H, 0, 0]);
    let (out_pk, out_spk, out_path) = derive(&[84 | H, H, H, 1, 0]);
    let (_, foreign_spk, _) = derive(&[84 | H, H, 99 | H, 0, 0]);

    let tx = Transaction {
        version: Version::TWO,
        lock_time: LockTime::ZERO,
        input: vec![
            TxIn { previous_output: OutPoint::new(Txid::from_byte_array([1; 32]), 0), sequence: Sequence::MAX, ..Default::default() },
            TxIn { previous_output: OutPoint::new(Txid::from_byte_array([2; 32]), 1), sequence: Sequence::MAX, ..Default::default() },
        ],
        output: vec![
    ...
    psbt.inputs[0].witness_utxo = Some(TxOut { value: Amount::from_sat(100_000), script_pubkey: in_spk });
    psbt.inputs[0].bip32_derivation.insert(in_pk.0, (fingerprint, in_path));
    psbt.inputs[1].witness_utxo = Some(TxOut { value: Amount::from_sat(60_000), script_pubkey: foreign_spk });
    psbt.outputs[0].bip32_derivation.insert(out_pk.0, (fingerprint, out_path));
    psbt
}
